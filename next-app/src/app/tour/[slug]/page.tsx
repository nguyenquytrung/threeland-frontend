...
import TimeLine from '@/components/pages/tour/Timeline';
import Hotels from '@/components/pages/destination-detail/Hotels';
import OurTerms from '@/components/pages/tour/OurTerms';
import Review from '@/components/pages/tour/review';
...
const Relates = dynamic(() => import('@/components/pages/tour/Relates'), {
...
export async function generateMetadata({
    ...
    const style = await getTour(params.slug);
...
const getTour = async (slug: string): Promise<TourModel> => {
    try {
        const apiUrl = Helper.apiRoutes(routes.tours.detail + slug);
        ...
        <div className='bg-[white] pt-2 max max-w-screen'>
                    ...
                    <div className='col-span-2 ml-2'>
                        ...
                        <div className='flex mt-2'>
                            {!!tour.images?.[2] && <Image
                                ...
                                alt={tour.images[2].name ?? ''}
                                width={288}
                                height={244}
                            />}
                            {!!tour.images?.[3] && <Image
                                // layout='responsive'
                                src={tour.images[3].url ?? ''}
                                className='rounded-[8px]  max-h-[244px] object-cover ml-2'
                                alt={tour.images[3].name ?? ''}
                                width={288}
                                height={244}
                            />}
                        </div>
            ...
            <section className='relative max-w-[1680px] px-[24px] sm:px-[50px] lg:px-[100px] mx-auto w-full pt-4 grid grid-cols-5'>
                <div className='col-span-3 pt-4'>
                    ...
                    <div className='flex items-center mt-2'>
                        <p className='text-[#0066B3] text-[40px] font-bold'>${tour.price}</p>
                    ...
                    <div
                        // className=' grid grid-cols-4 p-[20px]'
                        className=' mt-5'
                        dangerouslySetInnerHTML={{__html: DOMPurify.sanitize(tour.overview ?? '')}}
                    >
            ...
            <section className='bg-[#F9F9F9] pt-2'>
                <div className='relative items-center max-w-[1680px] px-[24px] sm:px-[50px] lg:px-[100px] mx-auto w-full block'>
                    ...
                    <div className='flex gap-[24px] items-center' dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(tour.highlight ?? '')}}>
                        {/* <Image
                            src={HightLightImage}
                            alt=''
            ...
            <section className='relative max-w-[1680px] px-[24px] sm:px-[50px] lg:px-[100px] mx-auto w-full pt-4'>
                ...
                <div className='grid grid-cols-3 gap-4'>
                    <div className='col-span-2'>
                        <TimeLine itineraries={tour.itineraries ?? []} />
                    </div>
