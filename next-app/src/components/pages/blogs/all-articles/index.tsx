...
const Index = ({ initialList, categories }: Props) => {
  ...
  const onViewMore = async () => {
    setLoading(true);
    const res = await fetch(Helper.apiRoutes(routes.blogs.list) + `?cursor=${list.meta.next_cursor}&category=${categoryId}`);
    setLoading(false);
    if (!res.ok) {
  ...
  const onChangeCategory = async (catId: number) => {
    ...
    setLoading(true);
    const res = await fetch(Helper.apiRoutes(routes.blogs.list) + `?category=${catId}`);
    setLoading(false);
    if (!res.ok) {
      console.log(res.statusText);
      throw new Error('Server Error');
  ...
  return (
    <div
      ...
      >
        ...
        <Categories categories={categories} categoryId={categoryId} onChange={onChangeCategory} />
      </div>
  );
};
