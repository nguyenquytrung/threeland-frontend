...
import In from '@/components/svg/inSocial';
...
const Index = ({ blog }: Props) => {
  ...
  return (
    <div className='lg:max-w-[1128px]'>
      <p className='leading-[35px] pb-10 text-justify'>{blog.description}</p>
          ...
          {blog.tableContent?.map((item, idx) => (
            <React.Fragment key={item.id}>
              <div className='flex mx-10 my-5'>
                ...
                <div>
                  <p
                    className='text-[20px] mx-5 text-[#0066B3] mb-5 cursor-pointer'
      ...
      <div
        dangerouslySetInnerHTML={{
          __html: DOMPurify.sanitize(blog.detail ?? ''),
        }}
      ></div>
      ...
      <div className='flex gap-4 flex-wrap mb-6'>
        ...
        <a
          className='self-center flex bg-[#3B5998] h-[48px] px-7 rounded-[4px]'
        ...
        <a
          className='self-center flex bg-[#d3d3d3] h-[48px] px-7 rounded-[4px]'
        ...
        <a
          className='self-center flex bg-[#FF0000] h-[48px] px-7 rounded-[4px]'
        ...
        <a
          className='self-center flex bg-[#007BB6] h-[48px] px-7 rounded-[4px]'
          href={`http://www.linkedin.com/shareArticle?mini=true&url=${url}&title=${blog.meta_title}&source=${process.env.NEXT_PUBLIC_BASE_URL}`}
          target='_blank'
        >
          <div className=' self-center rounded-[20px]'>
            <In />
